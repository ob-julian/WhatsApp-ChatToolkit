...
module.exports = {
    ...
    handler:  (qr, client, config) => {
        ...
        if (qrCodeTries === 0) {
            ...
            const elapsed = Math.floor((new Date() - time) / 1000);
            console.log(`Previous QR code expired after ${elapsed} seconds.`);
            time = new Date();
            console.log(`New QR Code received (try ${qrCodeTries + 1}), generating terminal QR code...`);
        }
        qrCodeTries++;
